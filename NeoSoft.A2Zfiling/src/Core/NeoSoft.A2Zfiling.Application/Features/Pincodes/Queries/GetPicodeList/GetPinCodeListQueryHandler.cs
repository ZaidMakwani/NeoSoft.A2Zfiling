...
namespace NeoSoft.A2Zfiling.Application.Features.Pincodes.Queries.GetPicodeList
{
    public class GetPinCodeListQueryHandler : IRequestHandler<GetPinCodeListQuery, Response<IEnumerable<PinCodeListVm>>>
    {
        ...
        public GetPinCodeListQueryHandler(IMapper mapper, ICategoryRepository categoryRepository, ILogger<GetPinCodeListQueryHandler> logger, IAsyncRepository<PinCode> stateRepsitory)
        {
            _mapper = mapper;
        ...
        public async Task<Response<IEnumerable<PinCodeListVm>>> Handle(GetPinCodeListQuery request, CancellationToken cancellationToken)
        {
            ...
            var allCategories = (await _pincodeRepsitory.ListAllAsync()).OrderBy(x => x.PinCodeNumber).Where(x=> x.IsActive == true);
            var category = _mapper.Map<IEnumerable<PinCodeListVm>>(allCategories);
            _logger.LogInformation("Hanlde Completed");
            return new Response<IEnumerable<PinCodeListVm>>(category, "success");
        }
